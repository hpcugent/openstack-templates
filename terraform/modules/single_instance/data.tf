...
resource "shell_script" "port_ssh" {
  environment = {
    ...
    "PORT_COUNT" = 1
    "PORT_NAME" = "${var.vm_name}_ssh"
    OS_CLOUD=var.cloud
  }
  lifecycle_commands {
    create = file("../scripts/generate_port.sh")
    delete = <<-EOF
      rm -rf "port_${var.vm_name}_ssh.json"
    EOF
    read = <<-EOF
      cat "port_${var.vm_name}_ssh.json"
    EOF
  }
...
resource "shell_script" "port_http" {
  ...
  environment = {
    "IP_ID" = data.openstack_networking_floatingip_v2.public.id
    "PORT_COUNT" = 1
    "PORT_NAME" = "${var.vm_name}_http"
  }
  lifecycle_commands {
    create = file("../scripts/generate_port.sh")
    delete = <<-EOF
      rm -rf "port_${var.vm_name}_http.json"
    EOF
    read = <<-EOF
      cat "port_${var.vm_name}_http.json"
    EOF
  }
  working_directory = "${path.root}"
  interpreter = ["/bin/bash","-c"]
