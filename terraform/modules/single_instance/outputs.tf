...
locals {
  ...
  http_string        = (var.nginx_enabled || var.expose_web) ? "HTTP: http://${data.openstack_networking_floatingip_v2.public.address}:${openstack_networking_portforwarding_v2.http[0].external_port}" : ""
  https_string       = (var.nginx_enabled || var.expose_web) ? "HTTPS: https://${data.openstack_networking_floatingip_v2.public.address}:${openstack_networking_portforwarding_v2.https[0].external_port}" : ""
